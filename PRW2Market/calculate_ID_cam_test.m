function [ID_cam_gallery, miss, result_cell_ind] = calculate_ID_cam_test(filename_cell, result_cell, anno_dir, img_index_test)

temp = [];
ID_cam_gallery = [];
miss = [];
result_cell_ind = cell(size(result_cell, 1), 1);
for n = 1:length(img_index_test)
    if round(n/1000) == n/1000
        fprintf('Test frame ID is %d \n',n);
    end
    
    box_data = result_cell{n};
    if isempty(box_data)
        continue;
    end
    
    % added
    box_data(:, 3:4) = box_data(:, 3:4) + box_data(:, 1:2);
    
    anno_data = importdata([anno_dir img_index_test{n} '.jpg.mat']); temp = [temp; anno_data];
    anno_data(:, 4:5) = anno_data(:, 4:5) + anno_data(:, 2:3);
    cam = str2double(img_index_test{n}(2));
    flag = zeros(size(anno_data, 1), 1);
    ID_cam_tmp = zeros(size(box_data, 1), 2);
    ov_tmp = zeros(size(box_data, 1), 1);
    for m = 1:size(box_data, 1)
        o = boxoverlap(anno_data(:, 2:5), box_data(m, 1:4));
        [m_v, pos] = max(o);
        ov_tmp(m) = m_v;
        if m_v >= 0.5
            ID_cam_tmp(m, :) = [anno_data(pos, 1), cam];
            flag(pos) = 1;
        end
        if m_v < 0.5 && m_v >= 0.3
            ID_cam_tmp(m, :) = [-1, cam];
        end
        if m_v < 0.3
            ID_cam_tmp(m, :) = [0, cam];
        end
    end
    
    % modified
    %ID_cam_tmp(:, 3) = box_data(:, 5);
    
    % ��֤detector����ID��bbox�������ظ�
    ID_cam_tmp(:, 4) = n;
    ID_uni = unique(ID_cam_tmp(:, 1));
    for m = 1:length(ID_uni)
        pos = find(ID_cam_tmp(:, 1) == ID_uni(m) & ID_uni(m) > 0);
        if length(pos) > 1
            ov_now = ov_tmp(pos);
            [mm, pp] = max(ov_now);
            pos(pp) = [];
            ID_cam_tmp(pos, 1) = -1;
        end
    end
    
    % modified
    result_cell_ind{n} = ID_cam_tmp(:, 1);
    
    ID_cam_gallery = [ID_cam_gallery; ID_cam_tmp];
    pos = find(flag == 0);
    anno_data = anno_data(pos, :);
    
    % modified
    %pos = find(anno_data(:, 1) > 0);
    %anno_data = anno_data(pos, :);
    
    tmp = [anno_data(: ,1), ones(size(anno_data, 1), 1).*cam];
    miss = [miss; tmp];
end
